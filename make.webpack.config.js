...
function makeConf(options) {
  ...
  var config = {
      ...
      sassLoader: {
        includePaths: [path.resolve(srcDir, 'css')]
      }
    }
    /**
...
var pages = fs.readdirSync(srcDir);
  ...
  if (dev) {
    ...
    var sassLoader = {
      test: /\.scss$/,
      loader: "style!css!sass?includePaths[]=" + proDir+"/node_modules/compass-mixins/lib"
    ...
    var sassLoader = {
      test: /\.scss$/,
      loader: ExtractTextPlugin.extract('style', 'css?-convertValues!sass?includePaths[]=' + proDir+"/node_modules/compass-mixins/lib")
    };
